...
1. 该策略 **仅允许** 用于**只读打开模式**（即未调用 `OpenOptions::write()`）。
2. 若文件以可写打开模式调用 `finish()` 并传递此策略，实现 **MUST** 返回策略冲突错误并拒绝打开。
...
1. 该策略 **仅允许** 用于**只读打开模式**（即未调用 `OpenOptions::write()`）。
2. 若文件以可写打开模式调用 `finish()` 并传递此策略，实现 **MUST** 返回策略冲突错误并拒绝打开。
3. 实现 **MUST** 跳过日志回放并允许 `finish()` 成功。
4. 实现 **MUST** 明确声明：该模式仅保证结构面读取，**不保证** 数据面一致性。
5. 若调用方进入数据面读取，结果 **MAY** 与“完成回放后的状态”不一致。
...
- [ ] `InMemoryOnReadOnly` 仅限只读打开模式、仅内存回放、不落盘；可写打开时返回策略冲突错误。  
- [ ] `ReadOnlyNoReplay` 仅限只读打开模式、明确标注“仅结构面保证”；可写打开时返回策略冲突错误。  
