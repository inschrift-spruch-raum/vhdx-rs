...
1. **字节序**：除特别声明外，均为小端。
2. **位序**：Bit 0 始终指最低有效字节的最低有效位（LSB0）。所有位域图示中 bit 0 在最左侧。
3. **CRC**：默认使用 CRC-32C（Castagnoli，多项式 0x1EDC6F41）。  
4. **对齐**：文件中主要对象按 **1 MB** 对齐；日志项按至少 **4 KB** 对齐。  
5. **不重叠**：BAT、Metadata、Log、Payload、Sector Bitmap 等必须互不重叠。  
6. **未知必需对象处理**：
